...
fn opencode_zen_context_window(model: &str) -> u64 {
    ...
    // GPT-5 family
    if m.contains("gpt-5") || m.contains("gpt-5") {
        return 200_000;
    }
    // GPT-4 family
    ...
    // Kimi
    if m.contains("kimi") || m.starts_with("k") {
        return 128_000;
    }
    // MiniMax
    ...
    // Nemotron
    if m.contains("nemotron") || m.contains("llama") {
        return 128_000;
    }
    // Big Pickle
    if m.contains("big-pickle") || m.contains("big_pickle") {
        return 128_000;
    }
