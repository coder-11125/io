use super::openai::{build_chat_body_with_model, parse_and_convert_chat_response, parse_and_convert_chunk};
...
impl AzureProvider {
    ...
    fn api_key(&self) -> anyhow::Result<String> {
        ...
        let env_var = self.config.api_key_env.as_deref().unwrap_or("AZURE_OPENAI_API_KEY");
    ...
    fn url(&self) -> anyhow::Result<String> {
        let endpoint = self.config.endpoint.clone()
            ...
            .ok_or_else(|| anyhow::anyhow!("Azure endpoint not set — set AZURE_OPENAI_ENDPOINT or config"))?;
        Ok(format!(
            "{}/openai/deployments/{}/chat/completions?api-version={}",
            endpoint.trim_end_matches('/'), self.config.deployment, self.config.api_version
        ))
    }
}
...
impl CompletionModel for AzureProvider {
    fn provider_name(&self) -> &'static str { "azure" }
    fn context_window(&self) -> u64 { super::context_window_for_model(&self.config.deployment) }
    ...
    async fn complete(&self, request: CompletionRequest) -> anyhow::Result<CompletionResponse> {
        ...
        let resp = self.client.post(url).header("api-key", api_key).json(&body).send().await?;
                    ...
                    Ok(bytes) => {
                        ...
                        while let Some(line_end) = buffer.find('\n') {
                            ...
                            match parse_and_convert_chunk(data) {
                                Ok(event) => { if tx.send(Ok(event)).await.is_err() { return; } }
